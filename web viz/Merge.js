...
var paper_Merge = new joint.dia.Paper({
  ...
  var pDocu_Merge = pFree_Merge.clone()
        .attr('.label/text', 'docu')
        .position(500, 50)
        .set('tokens', 0);
  var pBusy_Merge = pFree_Merge.clone()
        .attr('.label/text', 'busy')
        .position(350, 275)
        .set('tokens', 0);
    var pWait_Merge =  pFree_Merge.clone()
        .attr('.label/text', 'wait')
        .position(50, 175)
        .set('tokens', 5);
    var pInside_Merge = pFree_Merge.clone()
        .attr('.label/text', 'inside')
        .position(350, 175)
        .set('tokens', 0);
    var pDone_Merge = pFree_Merge.clone()
  ...
  var pStart_Merge= new pn1.Transition({
   ...
   var pChange_Merge = pStart_Merge.clone()
        .attr('id','change')
        .attr('.label/text', 'change')
        .position(525, 175);

    var pEnd_Merge = pStart_Merge.clone()
        .attr('id','end')
        .attr('.label/text', 'end')
        .position(370, 50); 
  ...
  graph_Merge.addCell([pWait_Merge,pInside_Merge,pDone_Merge,pFree_Merge,pDocu_Merge,pBusy_Merge,pStart_Merge,pChange_Merge,pEnd_Merge]);
  graph_Merge.addCell([
...
function checkValid(num){	
  var x = parseInt(num)	
  if(isNaN(x) || x<0) alert("Nhập số lớn hơn 0 giùm")	
  else return x	
}
...
paper_Merge.on('cell:pointerdblclick', function(cellView) {
    if(cellView.model.id == pChange_Merge.id || cellView.model.id == pStart_Merge.id || cellView.model.id == pEnd_Merge.id){
      alert("dm nhấp từ từ thoi, ai ăn của m à ¯\\_(ツ)_/¯");
  }
});
paper_Merge.on('cell:pointerdown', 
    function(cellView, evt, x, y) { 
        if(cellView.model.id==pChange_Merge.id)
           fireTransition(pChange_Merge, 1);
        else if(cellView.model.id==pStart_Merge.id)
           fireTransition(pStart_Merge, 1);
        else if(cellView.model.id==pEnd_Merge.id)
           fireTransition(pEnd_Merge, 1);
    }
);
