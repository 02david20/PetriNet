...
var paper_specialist = new joint.dia.Paper({
  ...
  var pDocu_specialist = pFree_specialist.clone()
        .attr('.label/text', 'docu')
        .position(450, 50)
        .set('tokens', 0);
  var pBusy_specialist = pFree_specialist.clone()
  ...
  var pStart_specialist= new pn1.Transition({
   ...
   var pChange_specialist = pStart_specialist.clone()
        .attr('id','change')
        .attr('.label/text', 'change')
        .position(475, 200);

    var pEnd_specialist = pStart_specialist.clone()
        .attr('id','end')
        .attr('.label/text', 'end')
        .position(320, 50); 
  ...
  graph_specialist.addCell([pFree_specialist,pDocu_specialist,pBusy_specialist,pStart_specialist,pChange_specialist,pEnd_specialist]);
...
function autoFirespecialist(){
  ...
  simulationspecialistId = simulate_specialist(pStart_specialist,pChange_specialist,pEnd_specialist);
}
...
paper_specialist.on('cell:pointerdblclick', function(cellView) {
    if(cellView.model.id == pChange_specialist.id || cellView.model.id == pStart_specialist.id || cellView.model.id == pEnd_specialist.id){
      alert("dm nhấp từ từ thoi, ai ăn của m à ¯\\_(ツ)_/¯");
  }
});
paper_specialist.on('cell:pointerdown', 
...
function simulate_specialist(pStart_specialist,pChange_specialist,pEnd_specialist) {
